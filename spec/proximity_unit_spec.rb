...
RSpec.describe ParadeDB::Proximity::RegexTerm do
  it "builds a prox regex wrapper" do
    regex_term = ParadeDB.prox_regex("sl.*", max_expansions: 100)
  ...
  it "rejects non-string patterns" do
    error = assert_raises(ArgumentError) { ParadeDB.prox_regex(123) }

    assert_includes error.message, "pattern must be a String"
  end

  it "rejects non-integer max expansions" do
    error = assert_raises(ArgumentError) { ParadeDB.prox_regex("sl.*", max_expansions: "100") }
