...
RSpec.describe "ArelIntegrationTest" do
  ...
  it "full matrix of operators" do
    ...
    nodes << @builder.phrase(:description, ParadeDB.slop("running shoes", 2))
    nodes << @builder.term(:description, ParadeDB.boost(ParadeDB.fuzzy("shose", 2, prefix: false), 2))
    ...
    nodes << @builder.near(:description, ParadeDB.proximity("sleek").within(1, "shoes"))
    ...
    assert_includes rendered, %("products"."category" @@@ pdb.term_set(ARRAY['audio', 'footwear']))
