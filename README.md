...
- The maintained minimum ParadeDB version is `0.21.10`; update `README.md`, `RELEASE.md`, and CI in the same PR whenever that floor changes.
...
MockItem.search(:description).near("running", "shoes", distance: 3)
MockItem.search(:description).near("running", "shoes", distance: 3, ordered: true)
MockItem.search(:description).near(["hiking", "running"], "shoes", distance: 2)
MockItem.search(:description).near("running", ["shoes", "sneakers"], distance: 2, ordered: true)
MockItem.search(:description).near(ParadeDB.regex_term("run.*"), "shoes", distance: 3)
